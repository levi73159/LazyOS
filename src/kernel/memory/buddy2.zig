...
pub fn Buddy2Allocator(comptime config: Config) type {
    return struct {
        ...
        fn alloc(ctx: *anyopaque, len: usize, alignment: mem.Alignment, ret_addr: usize) ?[*]u8 {
            ...
            const ptr = self.alignedAlloc(len, @intFromEnum(alignment));
        ...
        fn resize(ctx: *anyopaque, buf: []u8, old_align: mem.Alignment, new_len: usize, ret_addr: usize) bool {
            _ = old_align;
            _ = ret_addr;
            const self: *Self = @ptrCast(@alignCast(ctx));
        ...
        fn free(ctx: *anyopaque, buf: []u8, old_align: mem.Alignment, ret_addr: usize) void {
            _ = old_align;
            _ = ret_addr;
            const self: *Self = @ptrCast(@alignCast(ctx));
        ...
        fn remap(_: *anyopaque, memory: []u8, alignment: mem.Alignment, new_len: usize, ret_addr: usize) ?[*]u8 {
            _ = memory; // autofix
            _ = alignment; // autofix
            _ = new_len; // autofix
            _ = ret_addr; // autofix
